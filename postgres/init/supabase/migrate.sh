...
if [ "$#" -ne 0 ]; then
    export DATABASE_URL="${DATABASE_URL:-postgres://app_admin:$connect}"
    exec dbmate "$@"
    exit 0
fi
...
if [ -z "${USE_DBMATE:-}" ]; then
    ...
    for sql in "$db"/migrations/*.sql; do
        echo "$0: running $sql"
        psql -v ON_ERROR_STOP=1 --no-password --no-psqlrc -U authenticated -f "$sql"
    done
else
    # run init scripts as postgres user
...
psql -v ON_ERROR_STOP=1 --no-password --no-psqlrc -U authenticated -c 'SELECT extensions.pg_stat_statements_reset(); SELECT pg_stat_reset();' || true
