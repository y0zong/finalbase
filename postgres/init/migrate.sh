...
if [ "$#" -ne 0 ]; then
    export DATABASE_URL="${DATABASE_URL:-postgres://authenticated:$connect}"
    exec dbmate "$@"
    exit 0
fi
...
if [ -z "${USE_DBMATE:-}" ]; then
    ...
    for sql in "$db"/init-scripts/*.sql; do
        echo "$0: running $sql"
        psql -v ON_ERROR_STOP=1 -v PASSWORD=$PGPASSWORD --no-password --no-psqlrc -U postgres -f "$sql"
    done
    ...
    psql -v ON_ERROR_STOP=1 --no-password --no-psqlrc -U postgres -c "ALTER USER authenticated WITH PASSWORD '$PGPASSWORD'"
    # run migrations as super user - postgres user demoted in post-setup
    DBMATE_MIGRATIONS_DIR="$db/migrations" DATABASE_URL="postgres://authenticated:$connect" dbmate --no-dump-schema migrate
fi
...
if [ -e "$postinit" ]; then
    ...
    psql -v ON_ERROR_STOP=1 --no-password --no-psqlrc -U authenticated -f "$postinit"
fi

# once done with everything, reset stats from init
psql -v ON_ERROR_STOP=1 -v PASSWORD=$PGPASSWORD --no-password --no-psqlrc -U postgres -c 'SELECT extensions.pg_stat_statements_reset(); SELECT pg_stat_reset();' || true
